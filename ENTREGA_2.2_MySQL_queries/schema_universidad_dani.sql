...
SELECT DISTINCT PERSONA.NOMBRE, PERSONA.apellido1, PERSONA.apellido2 FROM alumno_se_matricula_asignatura INNER JOIN PERSONA ON ALUMNO_SE_MATRICULA_ASIGNATURA.id_alumno = PERSONA.ID INNER JOIN CURSO_ESCOLAR ON ALUMNO_SE_MATRICULA_ASIGNATURA.id_curso_escolar = CURSO_ESCOLAR.ID WHERE anyo_inicio = '2018' AND anyo_fin = '2019';



SELECT DEPARTAMENTO.NOMBRE, PERSONA.apellido1, PERSONA.apellido2, PERSONA.NOMBRE FROM DEPARTAMENTO RIGHT JOIN PROFESOR ON PROFESOR.id_departamento = DEPARTAMENTO.ID  RIGHT JOIN PERSONA ON PERSONA.ID = PROFESOR.ID_PROFESOR WHERE PERSONA.TIPO = 'PROFESOR' ORDER BY DEPARTAMENTO.NOMBRE, apellido1, apellido2, PERSONA.NOMBRE; 

SELECT * FROM PERSONA LEFT JOIN PROFESOR ON PERSONA.ID = PROFESOR.ID_PROFESOR WHERE PERSONA.TIPO= 'PROFESOR' AND id_departamento IS NULL;

SELECT * FROM DEPARTAMENTO LEFT JOIN PROFESOR ON DEPARTAMENTO.ID = PROFESOR.id_departamento WHERE ID_PROFESOR IS NULL;

SELECT PERSONA.* FROM profesor LEFT JOIN ASIGNATURA ON PROFESOR.ID_PROFESOR = ASIGNATURA.id_profesor LEFT JOIN PERSONA ON PROFESOR.id_profesor = PERSONA.ID WHERE ASIGNATURA.ID IS NULL;
...
SELECT DISTINCT DEPARTAMENTO.NOMBRE FROM PROFESOR RIGHT JOIN DEPARTAMENTO ON DEPARTAMENTO.ID = PROFESOR.id_departamento LEFT JOIN ASIGNATURA ON ASIGNATURA.id_profesor = PROFESOR.id_profesor WHERE ASIGNATURA.id_profesor IS NULL;

SELECT COUNT (*) AS NUMERO_ALUMNOS FROM PERSONA WHERE TIPO = 'ALUMNO';

SELECT COUNT (*) AS NUMERO_ALUMNOS FROM PERSONA WHERE TIPO = 'ALUMNO' AND fecha_nacimiento LIKE '1999%';

SELECT DEPARTAMENTO.NOMBRE, COUNT(*) AS CANTIDAD FROM DEPARTAMENTO INNER JOIN PROFESOR ON PROFESOR.id_departamento = DEPARTAMENTO.ID GROUP BY DEPARTAMENTO.NOMBRE HAVING COUNT(*) ORDER BY CANTIDAD ASC;

SELECT DEPARTAMENTO.NOMBRE, COUNT(ID_PROFESOR) AS PROFESORES FROM DEPARTAMENTO LEFT JOIN PROFESOR ON DEPARTAMENTO.ID = PROFESOR.id_departamento GROUP BY DEPARTAMENTO.nombre;


SELECT GRADO.NOMBRE, COUNT(ASIGNATURA.id) AS ASIGNATURAS FROM GRADO LEFT JOIN ASIGNATURA ON ASIGNATURA.id_grado = GRADO.id GROUP BY GRADO.NOMBRE ORDER BY ASIGNATURAS DESC;

SELECT GRADO.NOMBRE, COUNT(*) AS ASIGNATURAS FROM GRADO LEFT JOIN ASIGNATURA ON ASIGNATURA.id_grado = GRADO.id GROUP BY GRADO.NOMBRE HAVING COUNT(ASIGNATURA.ID) > 40;

SELECT GRADO.NOMBRE, ASIGNATURA.TIPO, SUM(ASIGNATURA.creditos) AS CREDITOS_TOTALES FROM GRADO INNER JOIN ASIGNATURA ON ASIGNATURA.id_grado = GRADO.id GROUP BY GRADO.NOMBRE, ASIGNATURA.TIPO;

SELECT CURSO_ESCOLAR.anyo_inicio, COUNT(alumno_se_matricula_asignatura.id_curso_escolar) FROM alumno_se_matricula_asignatura INNER JOIN curso_escolar ON alumno_se_matricula_asignatura.id_curso_escolar = CURSO_ESCOLAR.id INNER JOIN PERSONA ON alumno_se_matricula_asignatura.id_alumno = PERSONA.ID INNER JOIN ASIGNATURA ON alumno_se_matricula_asignatura.id_asignatura = ASIGNATURA.ID GROUP BY alumno_se_matricula_asignatura.id_curso_escolar;

SELECT PERSONA.ID, PERSONA.NOMBRE, PERSONA.apellido1, PERSONA.apellido2, COUNT(ASIGNATURA.nombre) AS NUMERO_ASIGNATURAS FROM asignatura RIGHT JOIN PERSONA ON ASIGNATURA.id_profesor = PERSONA.ID WHERE PERSONA.TIPO = 'PROFESOR' GROUP BY PERSONA.ID ORDER BY NUMERO_ASIGNATURAS DESC;

SELECT *, MIN(PERSONA.fecha_nacimiento) FROM PERSONA WHERE PERSONA.TIPO = 'ALUMNO';

SELECT * FROM PROFESOR INNER JOIN ASIGNATURA WHERE ASIGNATURA.ID_PROFESOR IS NULL GROUP BY PROFESOR.id_profesor;
